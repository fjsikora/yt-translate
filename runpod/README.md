# RunPod Multi-Container Deployment
...
```env
# Pre-built Workers
RUNPOD_WHISPER_ENDPOINT_ID=       # Deploy from RunPod Hub
RUNPOD_VLLM_ENDPOINT_ID=mo5vt1mlci0mtw  # ✅ Working

# Custom Workers (deploy after GitHub Actions builds images)
RUNPOD_DIARIZATION_ENDPOINT_ID=
...
1. Go to RunPod Console → Serverless → Explore
...
Using RunPod MCP or dashboard, create templates:
...
RUNPOD_WHISPER_ENDPOINT_ID=xxxxx
RUNPOD_DIARIZATION_ENDPOINT_ID=xxxxx
RUNPOD_DEMUCS_ENDPOINT_ID=xxxxx
RUNPOD_VLLM_ENDPOINT_ID=mo5vt1mlci0mtw
RUNPOD_CHATTERBOX_ENDPOINT_ID=xxxxx
...
### Test Diarization
```bash
curl -X POST "https://api.runpod.ai/v2/${RUNPOD_DIARIZATION_ENDPOINT_ID}/runsync" \
...
### Test Demucs
```bash
curl -X POST "https://api.runpod.ai/v2/${RUNPOD_DEMUCS_ENDPOINT_ID}/runsync" \
  -H "Authorization: Bearer ${RUNPOD_API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "input": {
...
### Test Chatterbox
```bash
curl -X POST "https://api.runpod.ai/v2/${RUNPOD_CHATTERBOX_ENDPOINT_ID}/runsync" \
  -H "Authorization: Bearer ${RUNPOD_API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "input": {
