...
Usage:
    This file is the entrypoint for the RunPod serverless worker.
...
def log_startup_info() -> None:
    ...
    print("=" * 60)
    print("RunPod Worker Starting - Health Check")
    print("=" * 60)
...
def validate_input(job_input: dict[str, Any]) -> tuple[str, str, Optional[str]]:
    ...
    Args:
        job_input: The job input dictionary from RunPod
...
def handler(job: dict[str, Any]) -> dict[str, Any]:
    ...
    This function is called for each job submitted to the RunPod endpoint.
    It translates the video and uploads the result to Supabase Storage.

    Args:
        job: RunPod job dictionary containing:
    ...
    try:
        # Validate inputs
        runpod.serverless.progress_update(job, "Validating inputs...")
        ...
        # Create progress callback for RunPod
        def progress_callback(info: ProgressInfo) -> None:
            ...
            runpod.serverless.progress_update(job, progress_msg)

        # Run translation
        runpod.serverless.progress_update(job, "Starting translation pipeline...")
        ...
        runpod.serverless.progress_update(job, "Translation complete!")
